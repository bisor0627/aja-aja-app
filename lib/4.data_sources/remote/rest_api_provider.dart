...
final restApiProvider = Provider<RestClient>(
  (ref) {
    ...
    // 토큰
    final accessToken = ref.watch(localRepositoryProvider).getAccessToken();
    // 리프레시 토큰
    final refreshToken = ref.watch(localRepositoryProvider).getRefreshToken();

    // dio 설정
    dio.options.baseUrl = F.endPoint;
      ...
      QueuedInterceptorsWrapper(
        ...
        onError: (DioException err, handler) async {
          ...
          if (statusCode == 401 && refreshToken != null) {
            ...
            //   // 토큰 저장
            //   ref.watch(localRepositoryProvider.notifier).setAccessToken(result.data!.accessToken);
            //   ref.watch(localRepositoryProvider.notifier).setRefreshToken(result.data!.refreshToken);
